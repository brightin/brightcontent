...
* Only exception: Page model is provided (with tree and draft / hidden). 
...
Implemented by <tt>find_partial</tt> function in Admin::ApplicationHelper
...
== Default actions
Default crud actions are included. @item (index) and @items are provided, next to the concrete names like @projects or @projects. Controllername should be ProjectController. You can overwrite the action in the controller.
...
== Add a new model to control in brightcontent
...
Add new controller in <tt>app/controllers/admin/projects_controller.rb</tt> and specify the fields you want to show in the index (list) and edit (form). For example:
