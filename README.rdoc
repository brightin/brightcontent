...
Run sync to add migrations and public assets (CSS etc) and run migrations
...
Add the resource projects to <tt>config/routes.rb</tt> under the *admin namespace*.

  resources :projects

Add a new controller in <tt>app/controllers/admin/projects_controller.rb</tt> .Inherit from <tt>Admin::ApplicationController</tt>. You can specify fields to show in index(list) and edit (form) by setting @list_fields and @form_fields in a before_filter. 
