...
=== DefaultViews
(mixin included in <tt>Admin::ApplicationController</tt>)
...
=== Default partials
(<tt>find_partial</tt> function in Admin Application Helper)
...
For example the field <tt>title</tt> will be rendered in the list by <tt>_list_field_title.html.erb</tt> if exists. Else it will be rendered with a normal label and textbox (because its a string).

== Default actions
Default crud actions are included. @item (index) and @items are provided, next to the concrete names like Project. Controllername should be ProjectController. You can overwrite them in your controller.

== Requirements
* Rails 3
* Uses ActiveRecord models to find the type of column/field to render (in generic field)
